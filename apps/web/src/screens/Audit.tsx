...
const ACTIONS = ["", "query", "summarize", "document_access", "login", "upload", "report_export", "config_model_provider", "verify_integrity"];
          ...
          <tbody>
            {(events.data ?? []).map((e: any) => (
              <tr key={e.seq} className="border-b border-line/60 hover:bg-surface-1/60">
                ...
                <td className="px-4 py-2 text-xs text-fg-low">
                  {e.action === "upload" && e.meta?.filenames
                    ? e.meta.filenames.map((fn: string, i: number) => (
                        <span key={i} className="block font-mono">{fn}</span>
                      ))
                    : <span className="font-mono">{e.target_type} {e.target_id?.slice(0, 8)}</span>}
                </td>
