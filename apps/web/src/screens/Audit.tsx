  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex items-end justify-between">
        <Header title="Audit Log" sub="Immutable · hash-chained · tamper-evident" />
        <div className="mb-5 flex gap-2">
          <button className="btn-ghost" onClick={verifyChain}><Fingerprint className="h-4 w-4" /> Verify chain</button>
          <button className="btn-ghost" onClick={exportCsv}><Download className="h-4 w-4" /> Export</button>
        </div>
      </div>
      ...
      {chain && (
        <Panel className={cx("mb-4 flex items-center gap-3 p-4", chain.valid ? "!border-ok/40" : "!border-critical/50")}>
          ...
          <div>
            <div className="font-display font-semibold text-fg-hi">
              {chain.valid ? "Hash chain intact" : "Chain broken — tampering detected"}
            </div>
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        {ACTIONS.map((a) => (
          <button key={a || "all"} onClick={() => setAction(a)} className={cx("chip", action === a && "chip-active")}>
            {a || "all"}
          </button>
        ))}
      </div>
      ...
      <Panel className="overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="border-b border-line bg-surface-2 font-mono text-[11px] uppercase tracking-wider text-fg-low">
            <tr>
          ...
          <tbody>
            {(events.data ?? []).map((e: any) => (
              <tr key={e.seq} className="border-b border-line/60 hover:bg-surface-2/40">
                ...
                <td className="px-4 py-2 font-mono text-xs text-signal">{e.hash}…</td>
          ...
          <>
            ...
            <div className="space-y-2">
              {d.sources.map((s: any, i: number) => (
                <div key={i} className="rounded-[3px] border border-line bg-surface-2/50 p-3">
