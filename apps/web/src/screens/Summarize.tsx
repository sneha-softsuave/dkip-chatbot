          ...
          <Panel className="p-5">
            <div className="mb-4 grid grid-cols-2 gap-2">
              {(["document", "topic"] as const).map((m) => (
                <button key={m} onClick={() => setMode(m)} className={cx("btn-ghost capitalize", mode === m && "!border-accent/50 !text-fg-hi !bg-surface-3")}>
                  {m}
                </button>
              ))}
            </div>
            ...
            {mode === "document" ? (
              <div>
                <label className="mb-1 block text-xs font-medium text-fg-mid">Document</label>
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-fg-mid">Topic</label>
        ...
        if (m) {
          ...
          return (
            <button key={i} onClick={() => c && onOpen(c)}
              className="mx-0.5 -translate-y-px rounded-full border border-accent/40 bg-accent/10 px-1.5 py-0.5 align-middle font-mono text-[11px] text-accent hover:bg-accent/20">
              {m[1]}
            </button>
          );
        }
