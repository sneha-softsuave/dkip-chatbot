...
export function Admin() {
  ...
  const [tab, setTab] = useState<"users" | "config">("config");
      ...
      {tab === "config" && (
        <Panel className="p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <StatusLed tone={health.data?.checks?.provider?.reachable !== false ? "ok" : "critical"} />
          </div>
          <div className="mb-3">
...
function UserManager() {
  ...
  const [form, setForm] = useState({ username: "", password: "", display_name: "", role: "user", clearance: "" as number | "" });

  const createUser = useMutation({
    mutationFn: (u: typeof form) => api.post("/users", { ...u, clearance: Number(u.clearance) }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["users"] }); setForm({ username: "", password: "", display_name: "", role: "user", clearance: "" }); },
  });
    ...
    <div className="space-y-4">
      <Panel className="p-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <input className="field" type="number" min={1} max={4} placeholder="Clearance level (1-4)" value={form.clearance}
                 onChange={(e) => setForm({ ...form, clearance: e.target.value === "" ? "" : Number(e.target.value) })} />
          <button className="btn-primary" onClick={() => createUser.mutate(form)}
                  disabled={!form.username || !form.password || !form.clearance || createUser.isPending}
                  title={!form.clearance ? "Select a clearance level" : undefined}>
