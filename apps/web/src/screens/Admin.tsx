...
import { Badge, HoloButton, InputField, PageHeader, Select, StatusLed, cx } from "../components/ui";
      ...
      <div className="mx-auto max-w-4xl">
        ...
        {tab === "config" && (
          <GlassPanel className="p-5" hover={false}>
            ...
            <div className="mb-3 text-xs text-fg-mid">
              Embed signature: {provider.data?.embed_signature ?? "…"} → Qdrant: {provider.data?.qdrant_collection ?? "…"}
            </div>
            <div className="flex gap-2">
              {(["cloud", "local"] as const).map((p) => (
                <HoloButton
                  ...
                  className={cx(provider.data?.provider === p && "opacity-60")}
                >
        ...
        {tab === "health" && (
          <GlassPanel className="p-5" hover={false}>
            ...
            {health.data?.checks &&
              Object.entries(health.data.checks as Record<string, any>).map(([name, c]) => (
...
function UserManager() {
  ...
  const createUser = useMutation({
    ...
    onSuccess: () => {
      ...
      setForm({ username: "", password: "", display_name: "", role: "user", clearance: 1 });
    },
  });
    ...
    <div className="space-y-4">
      <GlassPanel className="p-5" hover={false}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <InputField placeholder="Display name" value={form.display_name} onChange={(e) => setForm({ ...form, display_name: e.target.value })} />
          <Select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}>
            <option value="user">User</option>
            <option value="admin">Admin</option>
          </Select>
          <InputField type="number" min={1} max={4} placeholder="Clearance (1-4)" value={form.clearance} onChange={(e) => setForm({ ...form, clearance: Number(e.target.value) })} />
          <HoloButton onClick={() => createUser.mutate(form)} disabled={!form.username || !form.password || createUser.isPending}>
          ...
          {users.data?.map((u: any) => (
            <div key={u.subject} className="flex items-center justify-between rounded-md bg-surface-1 px-3 py-2">
              <div>
                ...
                <span className="ml-2 stamp text-fg-low">
                  {u.role} · CL{u.clearance}
                </span>
                {u.disabled && (
              ...
              {!u.disabled && (
                <HoloButton
                  variant="ghost"
                  className="!px-2 !py-1 text-xs text-critical hover:border-critical/50"
